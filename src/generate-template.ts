import type { RepositoryWithConfig, Template } from "~/types"
...
function generateTags(repoIndex: number, count: number): string[] {
	return Array.from({ length: count }, (_, j) => `v${repoIndex + 1}.${j}.0`)
}

function pickRepoName(index: number, usedNames: Set<string>): string {
...
function createRepository(index: number, tagsPerRepo: number, extraTags: number, usedNames: Set<string>): RepositoryWithConfig {
...
export function generateTemplate(repoCount: number, totalTags: number, hasAuth: boolean): Template {
...
export function generateTemplateFile(repoCount: number, totalTags: number, outputPath: string | undefined, hasAuth: boolean): void {
