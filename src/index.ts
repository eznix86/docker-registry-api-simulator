...
program
	...
	.action(async (options) => {
		...
		if (options.throttle !== undefined) {
			throttle = options.throttle === true ? 250 : Number.parseInt(options.throttle)
		}
		setImmediate(async () => {
...
program
	...
	.action(async (options) => {
		...
		const repoCount = Number.parseInt(options.repos, 10)
		const totalTags = Number.parseInt(options.tags, 10)
		const outputPath = options.output
		const hasAuth = !!options.auth

		if (Number.isNaN(repoCount) || repoCount <= 0) {
			console.error(chalk.red("Error: --repos must be a positive integer"))
			process.exit(1)
		}

		if (Number.isNaN(totalTags) || totalTags <= 0) {
