...
const RepositorySchema = v.object({
  name: v.pipe(
    ...
    v.regex(/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/, 'Repository name must be lowercase alphanumeric with optional separators')
  )
});
...
const ManifestSchema = v.object({
  type: v.picklist(['oci', 'docker', 'oci-index', 'docker-list'], 'Type must be one of: oci, docker, oci-index, docker-list'),
  data: v.object({
    ...
    manifests: v.optional(v.array(v.object({
      mediaType: v.string(),
      digest: v.pipe(v.string(), v.startsWith('sha256:')),
      size: v.pipe(v.number(), v.integer(), v.minValue(0)),
...
export function validateDatabase(data: unknown): void {
  try {
    ...
    if (error instanceof v.ValiError) {
      ...
      throw new Error('Database validation failed. Please check the errors above.');
    }
    throw error;
  }
}
...
export function validateSemantics(data: any): void {
  ...
  for (const [repoName, tags] of Object.entries(data.tags)) {
    ...
    for (const tag of tags as any[]) {
      if (!data.manifests[tag.digest]) {
        errors.push(`Repository "${repoName}" tag "${tag.tag}" references non-existent manifest ${tag.digest}`);
      }
    }
  }
  ...
  for (const repoName of Object.keys(data.tags)) {
    if (!data.repositories.some((r: any) => r.name === repoName)) {
      errors.push(`Repository "${repoName}" has tags but is not defined in repositories array`);
    }
  }

  // Check that manifest configs reference existing blobs
  for (const [digest, manifest] of Object.entries(data.manifests) as [string, any][]) {
    if (manifest.data.config?.digest) {
      if (!data.blobs[manifest.data.config.digest]) {
        errors.push(`Manifest ${digest} references non-existent config blob ${manifest.data.config.digest}`);
      }
    }
  }
      ...
      if (manifest.data.manifests) {
        for (const platformManifest of manifest.data.manifests) {
          if (!data.manifests[platformManifest.digest]) {
            errors.push(`Multi-arch manifest ${digest} references non-existent platform manifest ${platformManifest.digest}`);
          }
        }
      }
    }
  ...
  if (errors.length > 0) {
    ...
    throw new Error('Database semantic validation failed. Please check the errors above.');
  }
